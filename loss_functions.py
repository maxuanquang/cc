...
def spatial_normalize(disp):
    _mean = disp.mean(dim=1, keepdim=True).mean(
...
def photometric_flow_loss(tgt_img, ref_imgs, flows, explainability_mask, lambda_oob=0, qch=0.5, wssim=0.5, use_occ_mask_at_scale=False):
    ...
    def one_scale(explainability_mask, occ_masks, flows):
        ...
        ref_imgs_scaled = [nn.functional.adaptive_avg_pool2d(
            ref_img, (h, w)) for ref_img in ref_imgs]
        ...
        for i, ref_img in enumerate(ref_imgs_scaled):
            ...
            reconstruction_loss += (1 - wssim)*weight*oob_normalization_const*(robust_l1(
                diff, q=qch) + wssim*ssim_loss.mean()) + lambda_oob*robust_l1(1 - valid_pixels, q=qch)
            #weight /= 2.83
    ...
    for i in range(len(flows[0])):
        ...
        loss += one_scale(explainability_mask[i],
...
def photometric_reconstruction_loss(tgt_img, ref_imgs, intrinsics, intrinsics_inv, depth, explainability_mask, pose, rotation_mode='euler', padding_mode='zeros', lambda_oob=0, qch=0.5, wssim=0.5):
    def one_scale(depth, explainability_mask, occ_masks):
        assert(explainability_mask is None or depth.size()
               [2:] == explainability_mask.size()[2:])
        ...
        for i, ref_img in enumerate(ref_imgs_scaled):
            ...
            ref_img_warped = inverse_warp(
                ref_img, depth[:, 0], current_pose, intrinsics_scaled, intrinsics_scaled_inv, rotation_mode, padding_mode)
            valid_pixels = 1 - (ref_img_warped == 0).prod(1,
                                                          keepdim=True).type_as(ref_img_warped)
            diff = (tgt_img_scaled - ref_img_warped) * valid_pixels
            ssim_loss = 1 - ssim(tgt_img_scaled, ref_img_warped) * valid_pixels
            ...
            if explainability_mask is not None:
                diff = diff * (1 - occ_masks[:, i:i+1]) * \
                    explainability_mask[:, i:i+1].expand_as(diff)
                ssim_loss = ssim_loss * \
                    (1 - occ_masks[:, i:i+1]) * \
                    explainability_mask[:, i:i+1].expand_as(ssim_loss)
            else:
...
def census_loss(tgt_img, ref_imgs, flows, explainability_mask, lambda_oob=0, qch=0.5, wssim=0.5, use_occ_mask_at_scale=False):
    def abs_robust_loss_torch(x, eps=0.01, q=0.4):
        x = torch.pow((torch.abs(x) + eps), q)
        return x

    def zero_mask_border_torch(mask_b3hw, patch_size):
...
def depth_occlusion_masks(depth, pose, intrinsics, intrinsics_inv):
    flow_cam = [pose2flow(depth.squeeze(), pose[:, i], intrinsics,
                          intrinsics_inv) for i in range(pose.size(1))]
...
def explainability_loss(mask):
    ...
    loss = 0
    for mask_scaled in mask:
        ones_var = Variable(torch.ones(1)).expand_as(
            mask_scaled).type_as(mask_scaled)
...
def consensus_exp_masks(cam_flows_fwd, cam_flows_bwd, flows_fwd, flows_bwd, tgt_img, ref_img_fwd, ref_img_bwd, wssim, wrig, ws=0.1):
    def one_scale(cam_flow_fwd, cam_flow_bwd, flow_fwd, flow_bwd, tgt_img, ref_img_fwd, ref_img_bwd, ws):
        ...
        valid_pixels_cam_fwd = 1 - \
            (cam_warped_im_fwd == 0).prod(
                1, keepdim=True).type_as(cam_warped_im_fwd)
        valid_pixels_cam_bwd = 1 - \
            (cam_warped_im_bwd == 0).prod(
                1, keepdim=True).type_as(cam_warped_im_bwd)
        # if one of them is valid, then valid
        valid_pixels_cam = logical_or(
            valid_pixels_cam_fwd, valid_pixels_cam_bwd)

        valid_pixels_flow_fwd = 1 - \
            (flow_warped_im_fwd == 0).prod(
                1, keepdim=True).type_as(flow_warped_im_fwd)
        valid_pixels_flow_bwd = 1 - \
            (flow_warped_im_bwd == 0).prod(
                1, keepdim=True).type_as(flow_warped_im_bwd)
        # if one of them is valid, then valid
        valid_pixels_flow = logical_or(
            valid_pixels_flow_fwd, valid_pixels_flow_bwd)
        ...
        flow_err = (1-wssim)*robust_l1_per_pix(tgt_img_scaled - flow_warped_im_fwd).mean(1, keepdim=True) \
            + wssim*(1 - ssim(tgt_img_scaled, flow_warped_im_fwd)
                     ).mean(1, keepdim=True)
    ...
    for i in range(len(cam_flows_fwd)):
        exp_masks_target.append(one_scale(
            cam_flows_fwd[i], cam_flows_bwd[i], flows_fwd[i], flows_bwd[i], tgt_img, ref_img_fwd, ref_img_bwd, ws))
        ws = ws / 2.3
...
def compute_joint_mask_for_depth(explainability_mask, rigidity_mask_bwd, rigidity_mask_fwd, THRESH):
    ...
    for i in range(len(explainability_mask)):
        ...
        rigidity_mask_fwd_one_scale = (
            rigidity_mask_fwd[i] > THRESH).type_as(exp_mask_one_scale)
        rigidity_mask_bwd_one_scale = (
            rigidity_mask_bwd[i] > THRESH).type_as(exp_mask_one_scale)
        exp_mask_one_scale_joint = 1 - \
            (1-exp_mask_one_scale[:, 1]) * \
            (1-exp_mask_one_scale[:, 2]).unsqueeze(1) > 0.5
        joint_mask_one_scale_fwd = logical_or(rigidity_mask_fwd_one_scale.type_as(
            exp_mask_one_scale), exp_mask_one_scale_joint.type_as(exp_mask_one_scale))
        joint_mask_one_scale_bwd = logical_or(rigidity_mask_bwd_one_scale.type_as(
            exp_mask_one_scale), exp_mask_one_scale_joint.type_as(exp_mask_one_scale))
        joint_mask_one_scale_fwd = Variable(
            joint_mask_one_scale_fwd.data, requires_grad=False)
        joint_mask_one_scale_bwd = Variable(
            joint_mask_one_scale_bwd.data, requires_grad=False)
...
def consensus_depth_flow_mask(explainability_mask, census_mask_bwd, census_mask_fwd, exp_masks_bwd_target, exp_masks_fwd_target, THRESH, wbce):
    ...
    for i in range(len(explainability_mask)):
        exp_mask_one_scale = explainability_mask[i]
        census_mask_fwd_one_scale = (census_mask_fwd[i] < THRESH).type_as(
            exp_mask_one_scale).prod(dim=1, keepdim=True)
        census_mask_bwd_one_scale = (census_mask_bwd[i] < THRESH).type_as(
            exp_mask_one_scale).prod(dim=1, keepdim=True)
        ...
        census_mask_fwd_one_scale = logical_or(
            census_mask_fwd_one_scale, exp_fwd_target_one_scale)
        census_mask_bwd_one_scale = logical_or(
            census_mask_bwd_one_scale, exp_bwd_target_one_scale)
        ...
        census_mask_fwd_one_scale = Variable(
            census_mask_fwd_one_scale.data, requires_grad=False)
        census_mask_bwd_one_scale = Variable(
            census_mask_bwd_one_scale.data, requires_grad=False)
        ...
        loss += weighted_binary_cross_entropy(
            exp_mask_one_scale, rigidity_mask_combined.type_as(exp_mask_one_scale), [wbce, 1-wbce])
...
def flow_diff(gt, pred):
    ...
    diff = torch.sqrt(torch.pow((u_gt - u_pred), 2) +
                      torch.pow((v_gt - v_pred), 2))
...
def compute_epe(gt, pred):
    ...
    epe = torch.sqrt(torch.pow((u_gt - u_pred), 2) +
                     torch.pow((v_gt - v_pred), 2))
...
def compute_all_epes(gt, rigid_pred, non_rigid_pred, rigidity_mask, THRESH=0.5):
    ...
    rigidity_pred_mask = nn.functional.upsample(
        rigidity_mask, size=(h_pred, w_pred), mode='bilinear')
    rigidity_gt_mask = nn.functional.upsample(
        rigidity_mask, size=(h_gt, w_gt), mode='bilinear')

    non_rigid_pred = (rigidity_pred_mask <= THRESH).type_as(
        non_rigid_pred).expand_as(non_rigid_pred) * non_rigid_pred
    rigid_pred = (rigidity_pred_mask > THRESH).type_as(
        rigid_pred).expand_as(rigid_pred) * rigid_pred
